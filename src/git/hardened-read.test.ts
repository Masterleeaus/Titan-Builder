...
test('sanitizes inherited Git configuration and forces non-interactive behavior', () => {
...
test('hardened status disables repository fsmonitor and ignores aliases and environment injection', () => {
  ...
  assert.deepEqual(invocation.args.slice(-3), ['status', '--short', '--branch']);
...
test('hardened diff disables external diff and textconv drivers', () => {
  ...
  assert.equal(existsSync(sentinel), false);
  assert.deepEqual(invocation.args.slice(-3), ['diff', '--no-ext-diff', '--no-textconv']);
  assert.ok(invocation.args.includes('diff.external='));
});

test('automatic worktree inspection fails closed before configured content filters execute', async () => {
  const root = createRepository();
  const { command, sentinel } = createHelper(root);
