...
test('CLI exposes project, memory, and context commands and delegates prompt context to the shared workflow', async () => {
  const [source, workflowSource] = await Promise.all([
    readFile(new URL('../index.ts', import.meta.url), 'utf8'),
    readFile(new URL('../workflows/agent-preparation.ts', import.meta.url), 'utf8'),
  ]);
  ...
  assert.match(source, /prepareAgentRun\(/);
  assert.match(source, /runAskWorkflow\(/);
  assert.match(workflowSource, /formatProjectMemory\(memoryEntries\)/);
  assert.match(
    workflowSource,
    /formatProjectMemory\(await readProjectMemory\(projectRoot\)\)/,
  );
  assert.match(workflowSource, /buildContext\(projectRoot, contextRefs/);
