...
let running = false;
let compatibilityMode = { superpower: true };
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  if (message?.type === 'OPENBROWSER_JOB' || message?.type === 'OPENBROWSER_RUN_JOB') {
    void handleIncomingJob(message.job).catch((error) => {
      console.error('[openbrowser] queued job failed', error);
    });
    sendResponse?.({ ok: true, accepted: true });
    return false;
  }

  if (message?.type === 'OPENBROWSER_PROMPT') {
    void handleLibraryPrompt(message)
      .then((result) => sendResponse?.({ ok: true, ...result }))
      .catch((error) => sendResponse?.({ ok: false, error: String(error?.message ?? error) }));
    return true;
  }

  if (message?.type === 'OPENBROWSER_CHATGPT_SCAN') {
    void handleChatGPTScan(message.scan)
      .then((items) => sendResponse?.({ ok: true, items }))
      .catch((error) => sendResponse?.({ ok: false, error: String(error?.message ?? error) }));
    return true;
  }

  if (message?.type === 'OPENBROWSER_CHATGPT_FETCH_FILE') {
    void handleChatGPTFileFetch(message.url)
      .then((result) => sendResponse?.({ ok: true, ...result }))
      .catch((error) => sendResponse?.({ ok: false, error: String(error?.message ?? error) }));
    return true;
  }
...
async function handleLibraryPrompt(message) {
  ...
  compatibilityMode = {
    superpower: message?.compatibility?.superpower !== false,
  };
  ...
  if (message?.submit === true) {
    ...
    void runAutoContinue({
      ...
      mode: 'ask',
      scope: 'panel',
      settings: message.autoContinue,
      captureText: false,
      initialResponseComplete: false,
    }).catch((error) => console.warn('[openbrowser] auto-continue stopped', error));
  }
...
async function handleIncomingJob(job) {
  ...
  if (running) {
    if (!jobQueue.some((queued) => queued?.sessionId === job.sessionId)) {
      jobQueue.push(job);
    }
    return;
  }
...
async function runJob(job) {
  ...
  try {
    ...
    if (!claim.claimed || !claim.job || !claim.claimToken) {
      return;
    }

    claimToken = claim.claimToken;
    heartbeat = startClaimHeartbeat(job.sessionId, claimToken);
    processedSessionIds.add(job.sessionId);
    compatibilityMode = {
      superpower: job.compatibility?.superpower !== false,
    };
    await processJob({
      ...claim.job,
      claimToken,
    ...
    if (claimToken) {
      const reported = await postBrowserResponse({
        sessionId: job.sessionId,
        claimToken,
        error: String(error),
      }).then(() => true).catch(() => false);
      if (!reported) {
        processedSessionIds.delete(job.sessionId);
      }
    }
  } finally {
    if (heartbeat) {
...
async function claimJob(sessionId) {
  return bridgeRequest('/browser/claim', {
    method: 'POST',
    body: JSON.stringify({ sessionId, claimantId }),
  });
}

function startClaimHeartbeat(sessionId, claimToken) {
  const interval = setInterval(() => {
    void bridgeRequest('/browser/heartbeat', {
      method: 'POST',
      body: JSON.stringify({ sessionId, claimToken }),
    }).catch((error) => {
      console.warn('[openbrowser] claim heartbeat failed', error);
    });
  }, 30_000);
  return interval;
}
...
async function processJob(job) {
  ...
  let text = await waitForPlainResponse(beforeCount, job.mode, job.sessionId, {
    markdownDraft: job.markdownDraft,
    claimToken: job.claimToken,
  });
  if (job.mode === 'ask') {
    const continued = await runAutoContinue({
      beforeCount: countAssistantMessages(),
      mode: job.mode,
      scope: 'job',
      settings: job.autoContinue,
      captureText: true,
      initialResponseComplete: true,
      initialText: text,
    });
    if (continued?.length) text = [text, ...continued].filter(Boolean).join('\n\n');
  }
  if (job.mode === 'ask' && job.sessionId) {
    await postBrowserChunk({ sessionId: job.sessionId, claimToken: job.claimToken, text });
  }
...
function findSendButton() {
  ...
  const visible = candidates.filter(
    (button) => isElementVisible(button) && !isCompatibilityExcludedNode(button),
  );
  ...
  while (Date.now() < deadline) {
    ...
    if (text) {
      ...
      if (mode === 'ask' && sessionId && shouldPostChunk(text, lastChunkText, lastChunkAt)) {
        await postBrowserChunk({ sessionId, claimToken: options.claimToken, text });
        lastChunkText = text;
        ...
        if (Date.now() - stableSince >= stableMs && canFinishResponse()) {
          ...
          if (recheck && recheck.length >= text.length && canFinishResponse()) {
            if (mode === 'ask' && sessionId && recheck !== lastChunkText) {
              await postBrowserChunk({ sessionId, claimToken: options.claimToken, text: recheck });
            }
            return recheck;
          }
...
function findPromptInput() {
  ...
  const candidates = queryAll(provider.selectors.input);
  const preferred = candidates.find(
    (node) => isElementVisible(node) && !isCompatibilityExcludedNode(node),
  );
  return preferred ?? candidates.find((node) => !isCompatibilityExcludedNode(node)) ?? null;
}

function isCompatibilityExcludedNode(node) {
  if (!compatibilityMode.superpower || !node?.closest) {
    return false;
  }

  return Boolean(
    node.closest(
      '[data-superpower], [data-extension*="superpower" i], [id*="superpower" i], [class*="superpower" i], [aria-label*="superpower" i]',
    ),
  );
}
...
async function handleChatGPTScan(kind) {
  if (!/^(?:chatgpt\.com|chat\.openai\.com)$/.test(location.hostname)) {
    throw new Error('This scan is available only on a ChatGPT page.');
  }
  const tools = globalThis.OpenBrowserChatGPTTools;
  if (!tools) throw new Error('ChatGPT page tools are unavailable. Reload the tab.');
  if (kind === 'apps') return tools.scanVisibleApps(document);
  if (kind === 'files') return tools.scanVisibleFiles(document);
  if (kind === 'replies') return tools.scanConversationReplies(document);
  throw new Error('Unknown ChatGPT scan type.');
}

async function handleChatGPTFileFetch(rawUrl) {
  if (!/^(?:chatgpt\.com|chat\.openai\.com)$/.test(location.hostname)) {
  ...
  if (!allowed) throw new Error('The visible file URL is outside the allowed ChatGPT file hosts.');
...
function normalizeAutoContinueSettings(input = {}) {
  const rawMax = Number(input.maxContinuations ?? input.autoContinueMax);
  return {
    enabled: input.enabled === true,
    maxContinuations: Number.isFinite(rawMax) ? Math.min(10, Math.max(1, Math.trunc(rawMax))) : 3,
    allowFallbackPrompt: input.allowFallbackPrompt === true,
    fallbackPrompt: String(input.fallbackPrompt ?? 'Continue from where you stopped. Do not repeat completed content.').trim(),
  };
}

async function runAutoContinue({ beforeCount, mode, scope, settings, captureText, initialResponseComplete = false, initialText = '' }) {
  const config = normalizeAutoContinueSettings(settings);
  if (!config.enabled || mode !== 'ask' || !['panel', 'job'].includes(scope)) return [];
  const captured = [];
  let responseCount = Number(beforeCount) || countAssistantMessages();
  let lastText = String(initialText ?? '');
  if (!initialResponseComplete) {
    lastText = await waitForPlainResponse(responseCount, 'ask', null).catch(() => '') || '';
    responseCount = countAssistantMessages();
  }
  for (let count = 0; count < config.maxContinuations; count += 1) {
    const nativeButton = findNativeContinueButton();
    let next = null;
    if (nativeButton) {
      const previousText = lastText || getCurrentAssistantText();
      nativeButton.click();
      next = await waitForContinuationChange(previousText).catch(() => null);
    } else if (config.allowFallbackPrompt && looksLikeTruncatedResponse(lastText)) {
      await injectPrompt(config.fallbackPrompt);
      await sleep(150);
      await clickSendWhenReady();
      next = await waitForPlainResponse(responseCount, 'ask', null).catch(() => null);
    } else {
      break;
    }
    if (!next) break;
    lastText = next;
    responseCount = countAssistantMessages();
    if (captureText) captured.push(next);
  }
  return captured;
}

function looksLikeTruncatedResponse(text) {
  const value = String(text ?? '').trim();
  if (!value) return false;
  const fences = (value.match(/```/g) ?? []).length;
  if (fences % 2 === 1) return true;
  if (/[,:;\-–—]$/.test(value) || /(?:\.\.\.|…)$/.test(value)) return true;
  if (/\b(?:continue|continued|truncated|cut off|more to follow)\b[^.!?]*$/i.test(value)) return true;
  const openBraces = (value.match(/[\[{(]/g) ?? []).length;
  const closedBraces = (value.match(/[\]})]/g) ?? []).length;
  return value.startsWith('{') && openBraces > closedBraces;
}

function getCurrentAssistantText() {
  const nodes = collectAssistantNodes();
  if (!nodes.length) return '';
  return buildAskCaptureText(nodes[nodes.length - 1]);
}

async function waitForContinuationChange(previousText) {
  const deadline = Date.now() + RESPONSE_TIMEOUT_MS;
  let changedText = '';
  let stableSince = 0;
  while (Date.now() < deadline) {
    const current = getCurrentAssistantText();
    if (current && current !== previousText) {
      if (current === changedText) {
        if (Date.now() - stableSince >= ASK_STABLE_MS && canFinishResponse()) {
          return current.startsWith(previousText) ? current.slice(previousText.length).trim() : current;
        }
      } else {
        changedText = current;
        stableSince = Date.now();
  ...
  return changedText.startsWith(previousText) ? changedText.slice(previousText.length).trim() : changedText;
}

function findNativeContinueButton() {
  const labels = ['continue generating', 'continue response', 'keep going', 'resume response'];
  for (const button of document.querySelectorAll('button, [role="button"]')) {
    if (!isElementVisible(button) || isCompatibilityExcludedNode(button)) continue;
    const text = `${button.getAttribute?.('aria-label') ?? ''} ${button.textContent ?? ''}`.replace(/\s+/g, ' ').trim().toLowerCase();
    if (labels.includes(text) || labels.some((label) => text === label || text.startsWith(`${label} `))) return button;
  }
  return null;
}
